"""
Bidirectional mRNA-miRNAiso Pathway Transformer Model
...
Key Features:
- Single unified model handles both mRNA→miRNAiso and miRNAiso→mRNA
...
class ModalityEmbedding(nn.Module):
    """
    Learnable embeddings that indicate modality type (mRNA vs miRNAiso).
    """
    
    def __init__(self, hidden_dim: int):
        super().__init__()
        
        # Two modality types: 0 = mRNA, 1 = miRNAiso
    ...
    def forward(self, modality_ids: torch.Tensor, seq_len: int) -> torch.Tensor:
        """
        Args:
            modality_ids: (batch_size,) - 0 for mRNA, 1 for miRNAiso
...
class BidirectionalPathwayTransformer(nn.Module):
    """
    Unified bidirectional transformer for mRNA ↔ miRNAiso translation.
    ...
    def __init__(
        ...
        """
        Args:
            n_genes: Number of genes (common between mRNA and miRNAiso)
            n_pathways: Number of pathways
            hidden_dim: Hidden dimension
    ...
    def forward(
        ...
        Args:
            ...
            source_modality: 0 for mRNA, 1 for miRNAiso
            target_modality: 0 for mRNA, 1 for miRNAiso
    ...
    def mrna_to_mirna(self, mrna_expression: torch.Tensor) -> torch.Tensor:
        """Convenience method: mRNA → miRNAiso."""
    ...
    def mirna_to_mrna(self, mirna_expression: torch.Tensor) -> torch.Tensor:
        """Convenience method: miRNAiso → mRNA."""
...
#     print(f"✓ mRNA → miRNAiso: {mrna_input.shape} → {mirna_output.shape}")
    
#     # miRNAiso → mRNA
#     mrna_output = model.mirna_to_mrna(mirna_output)
#     print(f"✓ miRNAiso → mRNA: {mirna_output.shape} → {mrna_output.shape}")
