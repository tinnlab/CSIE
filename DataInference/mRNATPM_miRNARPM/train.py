"""
Training Script for Bidirectional mRNA-miRNA Transformer

Features:
- Bidirectional training (both mRNA→miRNA and miRNA→mRNA)
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.config['epochs']):
            ...
            if epoch % self.config.get('eval_interval', 5) == 0:
                ...
                logger.info(f"Metrics - Corr(mRNA→miRNA): {metrics['corr_mrna2mirna']:.4f}, "
                           f"Corr(miRNA→mRNA): {metrics['corr_mirna2mrna']:.4f}")
