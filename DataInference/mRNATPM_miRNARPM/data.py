"""
Data loading and preprocessing for bidirectional mRNA-miRNA transformer.

Key points:
- Both mRNA and miRNA use gene-level features (same namespace)
- miRNA has fewer genes than mRNA (subset)
- We keep only common genes that appear in both mRNA and miRNA
- Samples must be aligned between mRNA and miRNA (paired data)

This module handles:
1. Loading mRNA and miRNA data from multiple TCGA datasets
2. Merging data across cancer types (keeping common features)
3. Aligning to common gene space (intersection of mRNA and miRNA genes)
4. Aligning samples between mRNA and miRNA
...
class TCGADataLoader:
    """
    Loads and preprocesses paired mRNA-miRNA data from TCGA datasets.
    
    Both mRNA and miRNA are at gene level, but miRNA may have fewer genes.
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple of (mRNA_dataframe, miRNA_dataframe) with:
        ...
        self.mirna_data = self._merge_cancer_types(mirna_dict, data_type="miRNA")
        
        logger.info(f"\nAfter merging across cancer types:")
        logger.info(f"  mRNA shape: {self.mrna_data.shape}")
        logger.info(f"  miRNA shape: {self.mirna_data.shape}")
        
        # Step 4: Align samples (keep only common samples between mRNA and miRNA)
        logger.info("\n" + "="*80)
        logger.info("ALIGNING SAMPLES")
        logger.info("="*80)
        self.mrna_data, self.mirna_data = self._align_samples(self.mrna_data, self.mirna_data)
        
        # Step 5: Align features (keep only common GENES between mRNA and miRNA)
        logger.info("\n" + "="*80)
    ...
    def _load_cancer_type_data(
        ...
        """
        Load mRNA and miRNA data from each cancer type folder.
        ...
        for folder in tcga_folders:
            ...
            if not mirna_path.exists():
                logger.warning(f"⚠️  Skipping {cancer_type}: miRNA.csv not found")
                continue
            ...
            try:
                ...
                logger.info(f"  Loaded miRNA: {mirna_df.shape[0]} samples × {mirna_df.shape[1]} genes")
    ...
    def _merge_cancer_types(
        ...
        Args:
            ...
            data_type: "mRNA" or "miRNA" (for logging)
    ...
    def _align_samples(
        ...
        """
        Keep only samples that appear in both mRNA and miRNA datasets.
        
        Args:
            mrna_df: mRNA dataframe
            mirna_df: miRNA dataframe
        
        Returns:
            Tuple of aligned (mRNA_df, miRNA_df)
        """
        logger.info(f"Before alignment:")
        logger.info(f"  mRNA samples: {len(mrna_df)}")
        logger.info(f"  miRNA samples: {len(mirna_df)}")
        
        # Find common samples
        common_samples = sorted(list(mrna_df.index.intersection(mirna_df.index)))
        
        if len(common_samples) == 0:
            raise ValueError("No common samples found between mRNA and miRNA!")
        ...
        logger.info(f"  Removed from miRNA: {len(mirna_df) - len(common_samples)}")
    ...
    def _align_genes(
        ...
        """
        Keep only genes that appear in both mRNA and miRNA datasets.
        This is the key step since miRNA has fewer genes than mRNA.
        ...
        logger.info(f"  Removed from miRNA: {len(mirna_df.columns) - len(common_genes)}")
    ...
    def normalize_data(
        ...
        Args:
            Train/val dataframes for mRNA and miRNA
        ...
        logger.info(f"miRNA - Train mean: {mirna_train_norm.mean().mean():.4f}, std: {mirna_train_norm.std().mean():.4f}")
...
class PairedExpressionDataset(Dataset):
    """
    PyTorch Dataset for paired mRNA-miRNA expression data.
    ...
    def __init__(
        ...
        Args:
            ...
            mirna_data: miRNA expression data (samples × genes, same gene space)
        """
        ...
        assert len(self.mrna) == len(self.mirna), "mRNA and miRNA must have same number of samples"
        assert self.mrna.shape[1] == self.mirna.shape[1], "mRNA and miRNA must have same number of features (genes)"
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            Tuple of (mRNA_expression, miRNA_expression) for the same sample
        """
...
#         print(f"miRNA batch shape: {mirna_batch.shape}")
#         print(f"mRNA range: [{mrna_batch.min():.4f}, {mrna_batch.max():.4f}]")
#         print(f"miRNA range: [{mirna_batch.min():.4f}, {mirna_batch.max():.4f}]")
#         break
