"""
Loss Functions for Bidirectional mRNA-miRNA Transformer

Three main loss components:
1. Reconstruction Loss: MSE between predicted and actual expression
2. Cycle Consistency Loss: mRNA→miRNA→mRNA should equal original mRNA
...
class CycleConsistencyLoss(nn.Module):
    ...
    - mRNA → miRNA → mRNA ≈ original mRNA
    - miRNA → mRNA → miRNA ≈ original miRNA
...
class BidirectionalTransformerLoss(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            mirna_batch: (batch_size, n_genes) - miRNA expression
        ...
        # ===== 1. Forward Direction: mRNA → miRNA =====
        mirna_pred = model.mrna_to_mirna(mrna_batch)
        
        # Reconstruction loss (mRNA → miRNA)
        ...
        # Pathway coherence loss (mRNA → miRNA)
        ...
        # ===== 2. Backward Direction: miRNA → mRNA =====
        mrna_pred = model.mirna_to_mrna(mirna_batch)
        
        # Reconstruction loss (miRNA → mRNA)
        ...
        # Pathway coherence loss (miRNA → mRNA)
        if self.use_pathway_loss:
        ...
        if compute_cycle and self.lambda_cycle > 0:
            ...
            # Backward cycle: miRNA → mRNA → miRNA
