"""
Data loading and preprocessing for bidirectional mRNA-meth450 transformer.

Key points:
- Both mRNA and meth450 use gene-level features (same namespace)
- meth450 has fewer genes than mRNA (subset)
- We keep only common genes that appear in both mRNA and meth450
- Samples must be aligned between mRNA and meth450 (paired data)

This module handles:
1. Loading mRNA and meth450 data from multiple TCGA datasets
2. Merging data across cancer types (keeping common features)
3. Aligning to common gene space (intersection of mRNA and meth450 genes)
4. Aligning samples between mRNA and meth450
...
class TCGADataLoader:
    """
    Loads and preprocesses paired mRNA-meth450 data from TCGA datasets.
    
    Both mRNA and meth450 are at gene level, but meth450 may have fewer genes.
    ...
    def __init__(
        ...
        self.meth_data: Optional[pd.DataFrame] = None
        ...
        self.meth_scaler: Optional[StandardScaler] = None
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple of (mRNA_dataframe, meth450_dataframe) with:
        ...
        mrna_dict, meth_dict = self._load_cancer_type_data(tcga_folders)
        ...
        self.meth_data = self._merge_cancer_types(meth_dict, data_type="meth450")
        
        logger.info(f"\nAfter merging across cancer types:")
        logger.info(f"  mRNA shape: {self.mrna_data.shape}")
        logger.info(f"  meth450 shape: {self.meth_data.shape}")
        
        # Step 4: Align samples (keep only common samples between mRNA and meth450)
        logger.info("\n" + "="*80)
        logger.info("ALIGNING SAMPLES")
        logger.info("="*80)
        self.mrna_data, self.meth_data = self._align_samples(self.mrna_data, self.meth_data)
        
        # Step 5: Align features (keep only common GENES between mRNA and meth450)
        logger.info("\n" + "="*80)
        logger.info("ALIGNING GENES (FEATURES)")
        logger.info("="*80)
        self.mrna_data, self.meth_data = self._align_genes(self.mrna_data, self.meth_data)
        ...
        if self.handle_missing != 'drop':
            ...
            self.meth_data = self._handle_missing_values(self.meth_data, self.handle_missing)
        
        # Step 8: Log transformation
        if self.log_transform:
            logger.info("\nApplying log1p transformation...")
            self.mrna_data = self._log_transform(self.mrna_data)
            self.meth_data = self._log_transform(self.meth_data)
        ...
        return self.mrna_data, self.meth_data
    ...
    def _load_cancer_type_data(
        ...
        """
        Load mRNA and meth450 data from each cancer type folder.
        
        Returns:
            Tuple of (mrna_dict, meth_dict) where keys are cancer types
        """
        logger.info("\n" + "="*80)
        ...
        for folder in tcga_folders:
            ...
            if not meth_path.exists():
                logger.warning(f"⚠️  Skipping {cancer_type}: meth450.csv not found")
                continue
            ...
            try:
                ...
                logger.info(f"  Loaded meth450: {meth_df.shape[0]} samples × {meth_df.shape[1]} genes")
                
                # Find common samples for this cancer type
                common_samples = mrna_df.index.intersection(meth_df.index)
    ...
    def _merge_cancer_types(
        ...
        Args:
            ...
            data_type: "mRNA" or "meth450" (for logging)
    ...
    def _align_samples(
        ...
        """
        Keep only samples that appear in both mRNA and meth450 datasets.
        
        Args:
            mrna_df: mRNA dataframe
            meth_df: meth450 dataframe
        
        Returns:
            Tuple of aligned (mRNA_df, meth450_df)
        """
        logger.info(f"Before alignment:")
        logger.info(f"  mRNA samples: {len(mrna_df)}")
        logger.info(f"  meth450 samples: {len(meth_df)}")
        
        # Find common samples
        common_samples = sorted(list(mrna_df.index.intersection(meth_df.index)))
        
        if len(common_samples) == 0:
            raise ValueError("No common samples found between mRNA and meth450!")
        
        # Align both dataframes to common samples
        mrna_aligned = mrna_df.loc[common_samples]
        meth_aligned = meth_df.loc[common_samples]
        ...
        logger.info(f"  Removed from meth450: {len(meth_df) - len(common_samples)}")
        
        return mrna_aligned, meth_aligned
    ...
    def _align_genes(
        ...
        """
        Keep only genes that appear in both mRNA and meth450 datasets.
        This is the key step since meth450 has fewer genes than mRNA.
        ...
        Returns:
            Tuple of gene-aligned (mRNA_df, meth450_df) with same columns
        """
        logger.info(f"Before gene alignment:")
        logger.info(f"  mRNA genes: {len(mrna_df.columns)}")
        logger.info(f"  meth450 genes: {len(meth_df.columns)}")
        
        # Find common genes (intersection)
        common_genes = sorted(list(set(mrna_df.columns).intersection(set(meth_df.columns))))
        
        if len(common_genes) == 0:
            raise ValueError("No common genes found between mRNA and meth450!")
        
        self.common_genes = common_genes
        
        # Keep only common genes for both
        mrna_aligned = mrna_df[common_genes]
        meth_aligned = meth_df[common_genes]
        ...
        logger.info(f"  Removed from meth450: {len(meth_df.columns) - len(common_genes)}")
    ...
    def _filter_small_cancer_types(self, min_samples: int = 2):
        ...
        if small_cancer_types:
            ...
            self.meth_data = self.meth_data.loc[samples_to_keep]
    ...
    def _print_data_summary(self):
        ...
        logger.info(f"\nmeth450 data:")
        logger.info(f"  Shape: {self.meth_data.shape}")
        logger.info(f"  Value range: [{self.meth_data.min().min():.4f}, {self.meth_data.max().max():.4f}]")
        logger.info(f"  Mean: {self.meth_data.mean().mean():.4f}")
        logger.info(f"  Std: {self.meth_data.std().mean():.4f}")
    ...
    def get_train_val_split(
        ...
        Returns:
            Tuple of (mrna_train, meth_train, mrna_val, meth_val)
        """
        if self.mrna_data is None or self.meth_data is None:
        ...
        meth_train = self.meth_data.iloc[train_idx]
        
        mrna_val = self.mrna_data.iloc[val_idx]
        meth_val = self.meth_data.iloc[val_idx]
        ...
        return mrna_train, meth_train, mrna_val, meth_val
    
    def normalize_data(
        self,
        mrna_train: pd.DataFrame,
        mrna_val: pd.DataFrame,
        meth_train: pd.DataFrame,
        meth_val: pd.DataFrame
    ) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        Args:
            Train/val dataframes for mRNA and meth450
        ...
        self.meth_scaler = StandardScaler()
        ...
        meth_train_norm = pd.DataFrame(
            self.meth_scaler.fit_transform(meth_train),
            index=meth_train.index,
            columns=meth_train.columns
        )
        ...
        meth_val_norm = pd.DataFrame(
            self.meth_scaler.transform(meth_val),
            index=meth_val.index,
            columns=meth_val.columns
        )
        
        logger.info("✓ Normalized validation data")
        logger.info(f"\nmRNA - Train mean: {mrna_train_norm.mean().mean():.4f}, std: {mrna_train_norm.std().mean():.4f}")
        logger.info(f"meth450 - Train mean: {meth_train_norm.mean().mean():.4f}, std: {meth_train_norm.std().mean():.4f}")
        
        return mrna_train_norm, meth_train_norm, mrna_val_norm, meth_val_norm


class PairedExpressionDataset(Dataset):
    """
    PyTorch Dataset for paired mRNA-meth450 expression data.
    ...
    def __init__(
        ...
        meth_data: Union[pd.DataFrame, np.ndarray]
    ):
        """
        Args:
            mrna_data: mRNA expression data (samples × genes)
            meth_data: meth450 expression data (samples × genes, same gene space)
        """
        ...
        if isinstance(meth_data, pd.DataFrame):
            self.meth = torch.FloatTensor(meth_data.values)
        else:
            self.meth = torch.FloatTensor(meth_data)
        
        assert len(self.mrna) == len(self.meth), "mRNA and meth450 must have same number of samples"
        assert self.mrna.shape[1] == self.meth.shape[1], "mRNA and meth450 must have same number of features (genes)"
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            Tuple of (mRNA_expression, meth450_expression) for the same sample
        """
        return self.mrna[idx], self.meth[idx]
...
def create_dataloaders(
    ...
    """
    train_dataset = PairedExpressionDataset(mrna_train, meth_train)
    val_dataset = PairedExpressionDataset(mrna_val, meth_val)
...
#     # Load all data
#     mrna_data, meth_data = loader.load_data()
    
#     # Split into train/val/test
#     mrna_train, meth_train, mrna_val, meth_val, mrna_test, meth_test = \
...
#             meth_train, meth_val, meth_test
#         )
    
#     # Create dataloaders
#     train_loader, val_loader, test_loader = create_dataloaders(
#         mrna_train, meth_train,
#         mrna_val, meth_val,
#         mrna_test, meth_test,
#         batch_size=32,
#         num_workers=4
...
#     print("="*80)
#     for mrna_batch, meth_batch in train_loader:
#         print(f"mRNA batch shape: {mrna_batch.shape}")
#         print(f"meth450 batch shape: {meth_batch.shape}")
#         print(f"mRNA range: [{mrna_batch.min():.4f}, {mrna_batch.max():.4f}]")
#         print(f"meth450 range: [{meth_batch.min():.4f}, {meth_batch.max():.4f}]")
#         break
