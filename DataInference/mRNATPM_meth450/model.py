"""
Bidirectional mRNA-meth450 Pathway Transformer Model
...
Key Features:
- Single unified model handles both mRNA→meth450 and meth450→mRNA
...
class ModalityEmbedding(nn.Module):
    """
    Learnable embeddings that indicate modality type (mRNA vs meth450).
    """
    
    def __init__(self, hidden_dim: int):
        super().__init__()
        
        # Two modality types: 0 = mRNA, 1 = meth450
    ...
    def forward(self, modality_ids: torch.Tensor, seq_len: int) -> torch.Tensor:
        """
        Args:
            modality_ids: (batch_size,) - 0 for mRNA, 1 for meth450
...
class BidirectionalPathwayTransformer(nn.Module):
    """
    Unified bidirectional transformer for mRNA ↔ meth450 translation.
    ...
    def __init__(
        ...
        """
        Args:
            n_genes: Number of genes (common between mRNA and meth450)
            n_pathways: Number of pathways
            hidden_dim: Hidden dimension
    ...
    def forward(
        ...
        Args:
            ...
            source_modality: 0 for mRNA, 1 for meth450
            target_modality: 0 for mRNA, 1 for meth450
    ...
    def mrna_to_meth(self, mrna_expression: torch.Tensor) -> torch.Tensor:
        """Convenience method: mRNA → meth450."""
        return self.forward(mrna_expression, source_modality=0, target_modality=1)
    
    def meth_to_mrna(self, meth_expression: torch.Tensor) -> torch.Tensor:
        """Convenience method: meth450 → mRNA."""
        return self.forward(meth_expression, source_modality=1, target_modality=0)
