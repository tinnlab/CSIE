"""
Training Script for Bidirectional mRNA-meth450 Transformer

Features:
- Bidirectional training (both mRNA→meth450 and meth450→mRNA)
...
class Trainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, (mrna_batch, meth_batch) in enumerate(pbar):
            # Move to device
    ...
    def validate(self) -> Dict[str, float]:
        ...
        for mrna_batch, meth_batch in tqdm(self.val_loader, desc="Validation"):
            # Move to device
            mrna_batch = mrna_batch.to(self.device)
            meth_batch = meth_batch.to(self.device)
    ...
    def evaluate_metrics(self) -> Dict[str, float]:
        ...
        for mrna_batch, meth_batch in self.val_loader:
            mrna_batch = mrna_batch.to(self.device)
            meth_batch = meth_batch.to(self.device)
            ...
            all_meth.append(meth_batch.cpu())
            all_mrna_pred.append(mrna_pred.cpu())
            all_meth_pred.append(meth_pred.cpu())
        
        # Concatenate
        all_mrna = torch.cat(all_mrna, dim=0).numpy()
        all_meth = torch.cat(all_meth, dim=0).numpy()
        all_mrna_pred = torch.cat(all_mrna_pred, dim=0).numpy()
        all_meth_pred = torch.cat(all_meth_pred, dim=0).numpy()
        ...
        # mRNA → meth450 correlation
        corrs_m2mi = []
        for i in range(len(all_meth)):
            corr, _ = pearsonr(all_meth[i], all_meth_pred[i])
            if not np.isnan(corr):
                corrs_m2mi.append(corr)
        metrics['corr_mrna2meth'] = np.mean(corrs_m2mi)
        
        # meth450 → mRNA correlation
        corrs_mi2m = []
        for i in range(len(all_mrna)):
            corr, _ = pearsonr(all_mrna[i], all_mrna_pred[i])
            if not np.isnan(corr):
                corrs_mi2m.append(corr)
        metrics['corr_meth2mrna'] = np.mean(corrs_mi2m)
        
        # MSE
        metrics['mse_mrna2meth'] = np.mean((all_meth - all_meth_pred) ** 2)
        metrics['mse_meth2mrna'] = np.mean((all_mrna - all_mrna_pred) ** 2)
    ...
    def save_checkpoint(self, is_best: bool = False, filename: Optional[str] = None):
        ...
        checkpoint = {
            ...
            'meth_scaler_mean': self.data_loader.meth_scaler.mean_,
            'meth_scaler_scale': self.data_loader.meth_scaler.scale_,
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.config['epochs']):
            ...
            if epoch % self.config.get('eval_interval', 5) == 0:
                ...
                logger.info(f"Metrics - Corr(mRNA→meth450): {metrics['corr_mrna2meth']:.4f}, "
                           f"Corr(meth450→mRNA): {metrics['corr_meth2mrna']:.4f}")
...
def main(config_dict=None):
    ...
    mrna_data, meth_data = data_loader.load_data()
