"""
Inference Script for Bidirectional mRNA-meth450 Transformer

Generate predictions using a trained model on new data.

Features:
- Load trained model from checkpoint
- Predict mRNA → meth450 or meth450 → mRNA
...
class Predictor:
    ...
    def _load_checkpoint(self):
        ...
        if 'common_genes' in checkpoint:
            ...
            meth_scaler.mean_ = checkpoint['meth_scaler_mean']
            meth_scaler.scale_ = checkpoint['meth_scaler_scale']
            meth_scaler.n_features_in_ = len(common_genes)
            ...
            self.data_loader.meth_scaler = meth_scaler
            ...
            mrna_data, meth_data = self.data_loader.load_data()
    ...
    def predict_mrna_to_meth(
        ...
        Returns:
            meth_pred: (n_samples, n_genes) - Predicted meth450 expression
        """
        # Normalize input
        ...
        # Predict
        meth_pred_normalized = self.model.mrna_to_meth(mrna_tensor)
        
        # Convert back to numpy
        meth_pred_normalized = meth_pred_normalized.cpu().numpy()
        
        # Inverse transform if requested
        if inverse_transform:
            meth_pred = self.data_loader.meth_scaler.inverse_transform(meth_pred_normalized)
        else:
            meth_pred = meth_pred_normalized
        
        return meth_pred
    
    @torch.no_grad()
    def predict_meth_to_mrna(
        self,
        meth_input: np.ndarray,
        inverse_transform: bool = True
    ) -> np.ndarray:
        """
        Predict mRNA from meth450.
        
        Args:
            meth_input: (n_samples, n_genes) - meth450 expression (original scale)
            inverse_transform: Whether to transform back to original scale
        ...
        """
        # Normalize input
        meth_normalized = self.data_loader.meth_scaler.transform(meth_input)
        
        # Convert to tensor
        meth_tensor = torch.FloatTensor(meth_normalized).to(self.device)
        
        # Predict
        mrna_pred_normalized = self.model.meth_to_mrna(meth_tensor)
    ...
    def preprocess_new_data(
        ...
        Args:
            ...
            modality: 'mrna' or 'meth' (determines which scaler to use)
        ...
        if normalize_setting == 'no_norm':
            ...
            elif modality.lower() == 'meth':
                scaler = self.data_loader.meth_scaler
            else:
                raise ValueError(f"Unknown modality: {modality}. Use 'mrna' or 'meth'")
    ...
    def predict_new_data(
        ...
        Args:
            ...
            source_modality: 'mrna' or 'meth'
            target_modality: 'mrna' or 'meth'
        ...
        if source_modality.lower() == 'mrna' and target_modality.lower() == 'meth':
            pred_normalized = self.model.mrna_to_meth(input_tensor)
            source_scaler = self.data_loader.mrna_scaler
            target_scaler = self.data_loader.meth_scaler
        elif source_modality.lower() == 'meth' and target_modality.lower() == 'mrna':
            pred_normalized = self.model.meth_to_mrna(input_tensor)
            source_scaler = self.data_loader.meth_scaler
    ...
    def predict_batch(
        ...
        Args:
            ...
            meth_data: DataFrame with meth450 expression
            batch_size: Batch size for prediction
        
        Returns:
            Tuple of (meth_predictions, mrna_predictions)
        """
        # Convert to numpy
        mrna_array = mrna_data.values
        meth_array = meth_data.values
        
        n_samples = len(mrna_array)
        
        # Normalize
        mrna_normalized = self.data_loader.mrna_scaler.transform(mrna_array)
        meth_normalized = self.data_loader.meth_scaler.transform(meth_array)
        
        # Create dataset and loader
        dataset = PairedExpressionDataset(mrna_normalized, meth_normalized)
        ...
        for mrna_batch, meth_batch in tqdm(loader, desc="Predicting"):
            mrna_batch = mrna_batch.to(self.device)
            meth_batch = meth_batch.to(self.device)
            
            # mRNA → meth450
            meth_pred = self.model.mrna_to_meth(mrna_batch)
            all_meth_pred.append(meth_pred.cpu().numpy())
            
            # meth450 → mRNA
            mrna_pred = self.model.meth_to_mrna(meth_batch)
            all_mrna_pred.append(mrna_pred.cpu().numpy())
        
        # Concatenate
        meth_pred_normalized = np.concatenate(all_meth_pred, axis=0)
...
def main():
    ...
    mrna_train, meth_train, mrna_val, meth_val = \
    ...
    # Select split
    if args.split == 'train':
        mrna_data, meth_data = mrna_train, meth_train
    elif args.split == 'val':
        mrna_data, meth_data = mrna_val, meth_val
        ...
        meth_data = pd.concat([meth_train, meth_val])
    ...
    meth_pred, mrna_pred = predictor.predict_batch(
        mrna_data,
        meth_data,
    ...
    # Save mRNA → meth450 predictions
    predictor.save_predictions(
        meth_pred,
        mrna_data.index.tolist(),
        predictor.data_loader.common_genes,
        output_dir / f'{args.split}_mrna2meth_predictions.csv',
        format='csv'
    )
    
    # Save meth450 → mRNA predictions
    predictor.save_predictions(
        mrna_pred,
        meth_data.index.tolist(),
        predictor.data_loader.common_genes,
        output_dir / f'{args.split}_meth2mrna_predictions.csv',
        format='csv'
    )
    
    # Save ground truth for reference
    mrna_data.to_csv(output_dir / f'{args.split}_mrna_true.csv')
    meth_data.to_csv(output_dir / f'{args.split}_meth_true.csv')
